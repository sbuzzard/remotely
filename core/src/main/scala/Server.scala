...
object Server {
   ...
   * these failures.
   */
  def handle(env: Environment[_])(request: BitVector)(monitoring: Monitoring)(implicit S: Strategy): Task[BitVector] = {
    ...
    Task.delay(System.nanoTime).flatMap { startNanos => Task.suspend {
      ...
      val DecodeResult((respEncoder,ctx,r), trailing) = {
        codecs.requestDecoder(env).decode(request).fold(e => throw new Error(e.messageWithContext), identity)
      }
      ...
      if (unknown.nonEmpty) { // fail fast if the Environment doesn't know about some referenced values
        ...
        eval(env.values)(r)(S)(ctx).flatMap {
          a =>
  ...
  def eval[A](env: Values)(r: Remote[A])(implicit S: Strategy): Response[A] = {
    import Remote._
    r match {
